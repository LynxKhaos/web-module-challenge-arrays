...
function removeLastFlavor(takeOutFlavor){
  takeOutFlavor.pop();
   return takeOutFlavor
}
console.log(removeLastFlavor(copiedArr))
...
function getFlavorByIndex(arr, flavorTownIndex){
  return arr[flavorTownIndex] 
  }
console.log(getFlavorByIndex(copiedArr, 2))
...
function removeFlavorByName(flavorArr, flavorName){
  let index = flavorArr.indexOf(flavorName)
  flavorArr.splice(index,1)
  return flavorArr
  }
console.log(removeFlavorByName(copiedArr,'Peach'))
...
function filterByWord(arrOfFlavors, nameOfFlavor){
  let flavorArr = []
  for(let i = 0; i < arrOfFlavors.length; i++){
    if(arrOfFlavors[i].includes(nameOfFlavor)){
      flavorArr.push(arrOfFlavors[i])
    }
  }
  return flavorArr
  }
  console.log(filterByWord(copiedArr, 'Chocolate'))
